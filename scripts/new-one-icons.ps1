...
<#
.SYNOPSIS
    Generates original Zava Finance One icons for Teams and Microsoft 365.

.DESCRIPTION
    Writes only appPackage\one\color.png (192x192) and outline.png (32x32)
    ...
    one connected financial story, rather than the existing app's bar chart.
    The color icon uses a full-bleed teal (#087F8C) background, a white numeral,
    and a navy (#102A43) lower facet. Use #087F8C as the One app accent color.
...
[CmdletBinding()]
param(
    [string] $OutputDirectory = (Join-Path (Split-Path $PSScriptRoot -Parent) 'appPackage\one')
)
