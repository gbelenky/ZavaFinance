...
public sealed class AgentToolDeclarationTests
{
    ...
    public void DiscoversOnlyAnnotatedPublicInstanceMethodsOnSpecifiedClasses()
    {
        IReadOnlyList<AIFunctionDeclaration> tools =
            FinanceToolCatalog.Discover(typeof(SampleTools));
    ...
    {
        AIFunctionDeclaration tool = FinanceToolCatalog.Discover(typeof(SampleTools))
    ...
    {
        string[] names = [.. FinanceToolCatalog.Declarations.Select(t => t.Name).Order(StringComparer.Ordinal)];
    ...
    public void EachFunctionHasOnlyItsOwnArguments()
    {
        ...
        foreach (AIFunctionDeclaration tool in FinanceToolCatalog.Declarations)
        {
            ...
            Assert.False(tool is AIFunction, "Schema inspection must not bind caller services.");
        }
    }
    ...
    {
        string prompt = FinanceAgentFactory.SystemInstructions;
    ...
    public void NoRouteStillReachesTheModel()
    {
        ...
        Assert.Contains(
            FinanceToolNames.NoTool, FinanceAgentFactory.SystemInstructions, StringComparison.Ordinal);
    }
    ...
    {
        AIFunctionDeclaration statement = FinanceToolCatalog.Declarations.Single(
