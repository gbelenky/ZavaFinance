...
/// <summary>
/// Reuses the model client while each turn receives fresh invocation-budget middleware.
/// </summary>
...
#pragma warning disable OPENAI001 // Current hosted Responses adapter.
        Factory = new FinanceAgentFactory(
            projectClient.GetProjectOpenAIClient().GetResponsesClient()
                .AsIChatClient(FoundryTestEnvironment.ModelDeployment!),
            new FoundryOptions
            {
...
#pragma warning restore OPENAI001
    ...
    public FinanceAgentFactory? Factory { get; }
}
...
public sealed class RoutingEvalTests : IClassFixture<RoutingAgentFixture>
{
    ...
    public async Task RoutesGoldenSetCase(string caseId)
    {
        ...
        AIAgent agent = _fixture.Factory!.Create(FinanceTestTools.Create(), 20);
        ...
        {
            await agent.RunAsync(priorTurn, session, cancellationToken: cts.Token);
            agent = _fixture.Factory.Create(FinanceTestTools.Create(), 20);
        }
