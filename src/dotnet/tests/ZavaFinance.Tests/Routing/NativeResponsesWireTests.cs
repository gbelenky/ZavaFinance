...
#pragma warning restore OPENAI001
        ...
        AIAgent agent = factory.Create(FinanceTestTools.Create(), 20);
        AgentSession session = await agent.CreateSessionAsync();

        AgentResponse first = await agent.RunAsync("Show the figures", session);
        ...
        agent = factory.Create(FinanceTestTools.Create(), 20);
        session = await agent.DeserializeSessionAsync(saved);
        await agent.RunAsync("And for EMEA?", session);
        ...
        // Restored history rebuilds canonical markers instead of replaying stored result bodies.
        Assert.Equal(FinanceHistory.WithheldResult,
