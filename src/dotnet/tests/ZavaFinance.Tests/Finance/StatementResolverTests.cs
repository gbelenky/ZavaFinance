...
public sealed class StatementResolverTests
{
    ...
    [InlineData("Operating Margin %")]
    [InlineData("Operating Margin percent")]
    public void OptionalPercentUnitNeverChoosesBetweenCollidingKpis(string text)
    {
        var pending = new PendingClarification("request", "kpi", "v1", new(null, "", ""),
            ["amount", "rate"], DateTimeOffset.MaxValue, "user", ResolverTestData.Release,
            [ClarificationSelection.HashLabel("Operating Margin"), ClarificationSelection.HashLabel("Operating Margin %")]);

        Assert.True(ClarificationSelection.TryRead(text, pending, out var submission));
        Assert.Null(submission);
        Assert.True(ClarificationSelection.TryRead("option 2", pending, out submission));
        Assert.Equal("rate", submission!.OptionId);
    }

    [Theory]
    [InlineData("Operating")]
    [InlineData("Operating Marg")]
    [InlineData("Operating Margin per FTE")]
    [InlineData("Operating Margin for APAC")]
    [InlineData("Net sales")]
    public void TypedKpiChoicesRequireCompleteBoundTermsNotPrefixesOrUnlistedAliases(string text)
    {
        var pending = new PendingClarification("request", "kpi", "v1", new(null, "", ""),
            ["rate"], DateTimeOffset.MaxValue, "user", ResolverTestData.Release,
            [ClarificationSelection.HashLabel("Operating Margin %")],
            [[ClarificationSelection.HashLabel("Operating profitability")]]);

        Assert.False(ClarificationSelection.TryRead(text, pending, out _));
    }

    [Fact]
    public void OptionalPercentUnitDoesNotAlterOrganizationIdentity()
    {
        var pending = new PendingClarification("request", "org", "v1", new(null, "", ""),
            ["org"], DateTimeOffset.MaxValue, "user", ResolverTestData.Release,
            [ClarificationSelection.HashLabel("Operating Margin %")]);

        Assert.False(ClarificationSelection.TryRead("Operating Margin", pending, out _));
        Assert.True(ClarificationSelection.TryRead("Operating Margin %", pending, out var submission));
        Assert.Equal("org", submission!.OptionId);
    }
