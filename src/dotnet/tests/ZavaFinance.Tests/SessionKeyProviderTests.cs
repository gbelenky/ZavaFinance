...
/// never land on the same persisted agent session.
/// </summary>
public sealed class SessionKeyProviderTests
{
    ...
    [Fact]
    public void KeyUsesHmacWithUnambiguousLengthPrefixedIdentity()
    {
        const string oid = "11111111-1111-1111-1111-111111111111";
        using var input = new MemoryStream();
        foreach (string field in new[] { Tenant, oid, GroupChat })
        {
            byte[] bytes = Encoding.UTF8.GetBytes(field);
            input.Write(BitConverter.GetBytes(bytes.Length));
            input.Write(bytes);
        }
        byte[] expected = HMACSHA256.HashData(Encoding.UTF8.GetBytes(Salt), input.ToArray());

        Assert.Equal(SessionKeyProvider.ToBase32(expected), Provider.GetSessionKey(User(oid), GroupChat));
    }
    ...
    public void KeyIsAlwaysALegalStorageKey(string conversationId)
    {
    ...
    /// <summary>
    /// The partition key is visible in storage diagnostics, so it must not
