...
class CompositionTests(unittest.IsolatedAsyncioTestCase):
    ...
    def fixtures(self, settings):
        ...
        values = {
            ...
            "FoundryModel": MagicMock(return_value=self.model),
    ...
    async def test_composes_tools_and_closes_resources_after_host(self):
        ...
        kwargs = mocks["FinanceAgent"].call_args.kwargs
        self.assertEqual(mocks["FinanceAgent"].call_args.args, (settings, self.model.client, self.store))
        self.assertIs(kwargs["model_options"], self.model.options.return_value)
        tokens = object()
        for key, factory in (
        ...
        mocks["create_activity_host"].assert_called_once_with(
            settings, mocks["FinanceAgent"].return_value
        )
    ...
    async def test_unconfigured_sql_and_search_are_not_initialized(self):
        ...
        self.assertIsNone(mocks["FinanceAgent"].call_args.kwargs["search"])
        self.assertIsNone(mocks["FinanceAgent"].call_args.kwargs["query_factory"](object()))
