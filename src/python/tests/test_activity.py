...
from microsoft_agents.hosting.core import ConnectorClientBase, MemoryStorage, UserTokenClientBase

from zavafinance.activity import (
    CHOICE_FAILURE, IDENTITY_FAILURE, create_activity_host, normalize_continuation,
)
from zavafinance.channel.continuations import ContinuationQueue, QueueUnavailable, _ResponseGate, _response_gate
from zavafinance.channel.sdk_compat import ActivityDeliveryError
...
from zavafinance.contracts import (
    CallerIdentity, ClarificationOption, ClarificationPrompt, FinanceApplication, FinanceReply, IdentityError,
)
...
class FakeChannelFactory:
    ...
    def __init__(self):
        ...
        self.exchanges = []
        self.sign_in_resource = SignInResource(
    ...
    async def create_user_token_client(self, context, claims, anonymous):
        ...
        async def get_token_or_sign_in_resource(**kwargs):
            return SimpleNamespace(
                ...
                sign_in_resource=self.sign_in_resource,
            )
...
class ActivityHttpTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_host_depends_on_finance_application_protocol(self):
        self.assertIs(FinanceApplication, get_type_hints(create_activity_host)["orchestrator"])
    ...
    async def _pause_http_ack(self, fail_ack=False):
        waiting, release = asyncio.Event(), asyncio.Event()

        async def gated_host(scope, receive, send):
            async def gated_send(message):
                if message["type"] == "http.response.body" and not message.get("more_body", False):
                    waiting.set()
                    await release.wait()
                    if fail_ack:
                        raise OSError("Synthetic response disconnect")
                await send(message)

            await self.host(scope, receive, gated_send)

        async with httpx.AsyncClient(transport=httpx.ASGITransport(app=gated_host),
                                     base_url="https://native.test") as client:
            request = asyncio.create_task(client.post("/activity/messages", json=incoming(
                type="invoke", name="task/submit", value={"data": selection()})))
            try:
                await asyncio.wait_for(waiting.wait(), 2)
                await asyncio.sleep(0)
                self.assertEqual([], self.calls)
                self.assertEqual(1, len(self.factory.created))
                release.set()
                if fail_ack:
                    with self.assertRaisesRegex(OSError, "Synthetic response disconnect"):
                        await request
                else:
    ...
    async def test_finance_waits_for_http_ack_and_then_gets_fresh_token(self):
        await self._pause_http_ack()
        self.assertEqual(1, len(self.calls))
        self.assertEqual(2, len(self.factory.tokens))
        self.assertNotEqual(self.factory.tokens[0], self.calls[0][2])
        self.assertEqual(self.factory.tokens[-1], self.calls[0][2])

    async def test_failed_http_ack_discards_queued_finance_work(self):
        await self._pause_http_ack(fail_ack=True)
        self.assertEqual([], self.calls)
        self.assertEqual(1, len(self.factory.created))
    ...
    async def test_oauth_card_omits_absent_optional_resources_and_resumes_question(self):
        self.factory.no_token = True
        self.factory.empty_oauth_ack = True
        self.factory.sign_in_resource = SignInResource(
            sign_in_link="https://login.microsoftonline.com/synthetic")
        self.assertEqual(202, (await self.post(incoming())).status_code)
        card = self.factory.sends[-1].attachments[0].content
        self.assertNotIn("tokenExchangeResource", card)
        self.assertNotIn("tokenPostResource", card)
        self.factory.no_token = False
        response = await self.post(incoming(type="invoke", name="signin/verifyState",
                                            value={"state": "123456"}))
        self.assertEqual(200, response.status_code, (response.text, self.errors))
        await self.drain()
        self.assertEqual("Revenue?", self.calls[-1][1])
    ...
    async def test_failed_pipeline_closes_both_native_turn_clients(self):
        self.factory.empty_ack = True
        self.assertEqual(500, (await self.post(incoming())).status_code)
        context = self.factory.created[-1]
        context.services.get(ConnectorClientBase).close.assert_awaited_once()
        context.services.get(UserTokenClientBase).close.assert_awaited_once()
    ...
    async def test_sso_client_failure_logs_only_safe_code_and_keeps_question(self):
        ...
        for code, expected in (("resourcematchfailed", "resourcematchfailed"),
            ...
            with patch("zavafinance.channel.sdk_compat.logger.warning") as warning:
...
class QueueTests(unittest.IsolatedAsyncioTestCase):
    async def test_ack_gate_cancellation_and_no_fire_and_forget(self):
        dispatch = AsyncMock()
        queue = ContinuationQueue(dispatch, capacity=1, workers=1)
        gate = _ResponseGate()
    ...
    async def test_shutdown_cancels_inflight_work_and_joins_workers(self):
        ...
        queue = ContinuationQueue(dispatch)
    ...
    async def test_worker_failure_is_observable_and_not_recovered(self):
        queue = ContinuationQueue(AsyncMock(side_effect=RuntimeError("synthetic failure")))
        gate = _ResponseGate(delivered=True)
        gate.ready.set()
        token = _response_gate.set(gate)
        try:
