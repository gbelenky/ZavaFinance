...
from zavafinance.model import RouteDecision, WITHHELD_RESULT
...
class FinanceAgentTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.tokens = object()
        self.app = FinanceAgent(self.settings, self.model.client, self.store,
                                model_options=self.model.options(), query_factory=self.query_factory,
    ...
    async def test_statement_receives_sticky_state_raw_arguments_and_utc_date(self):
        ...
        with patch("zavafinance.tools.StatementTool", return_value=tool) as constructor:
    ...
    async def test_pending_selection_delegates_binding_checks_and_never_adds_history(self):
        ...
        with patch("zavafinance.tools.StatementTool", side_effect=create):
    ...
    async def test_save_failure_preserves_completed_answer(self):
        ...
        with self.assertLogs("zavafinance.agent", level="ERROR") as logs:
