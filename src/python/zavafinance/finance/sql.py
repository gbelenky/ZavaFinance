...
def _money(value) -> Decimal:
    ...
    if not isinstance(value, (Decimal, int, float, str)) or isinstance(value, bool):
        raise ValueError("Expected a SQL numeric value.")
    # Fabric FLOAT columns are approximate; avoid introducing further binary
    # expansion when crossing into the calculator's decimal arithmetic.
...
class FabricStatementQuery:
    ...
    def _rows(connection, sql: str, parameters: tuple = (), *, limit: int, cancelled: threading.Event):
        ...
        try:
            if cancelled.is_set():
                raise InterruptedError()
            cursor.execute(sql, parameters, use_prepare=bool(parameters))
            # Guarded SQL batches can expose non-row results before the SELECT.
            while cursor.description is None:
                if cancelled.is_set():
                    raise InterruptedError()
                if not cursor.nextset():
                    raise ValueError("Finance SQL did not return a row result.")
            rows = []
            while True:
                if cancelled.is_set():
                    raise InterruptedError()
