"""Foundry model resources and strict, schema-compatible model message conversion."""
...
Never answer a KPI or statement question yourself. Never invent missing business arguments.
...
canonicalization. Resolve dateRange using the tool's date-resolution rules; those
rules permit contextual inheritance and the supplied current-year default.
Never convert a vague KPI to a particular KPI, drop an unknown date qualifier, or
broaden a local organization to a region or company.
...
def validate_decision(decision: RouteDecision) -> RouteDecision:
    ...
    if decision.name is None:
        ...
        return decision
    if not isinstance(decision.name, str) or decision.name not in TOOL_INPUTS:
    ...
    try:
        TOOL_INPUTS[decision.name].model_validate(decision.arguments)
    except ValidationError:
        raise ToolSelectionError("The function call has invalid arguments.") from None
    return decision


def read_selection(response: ChatResponse) -> RouteDecision:
...
class FoundryModel:
    """Own the Foundry connection, not an application-defined agent loop."""
    ...
    def options(self) -> OpenAIChatOptions:
        options: OpenAIChatOptions = {"model": self.settings.model_deployment, "store": False}
    ...
    async def rerank(self, query: str, kind: str, candidates: Sequence[ResolverEntity]) -> str | None:
        ...
        try:
            async with asyncio.timeout(self.settings.resolver_timeout_seconds):
                response = await self.client.get_response(messages, options=options)
