...
class SqlTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_no_rows_nulls_and_float_values_are_safe(self):
        ...
        float_row = (1.1, *COMPONENT_ROW[1:])
        with patch("mssql_python.connect", side_effect=[Connection([RELEASE_ROW]), Connection([float_row])]):
            with self.assertRaises(ValueError):
    ...
    async def test_decimal_headcount_cast_matches_dotnet_and_overflow_fails(self):
        ...
        for headcount in (1.5, True, D(2 ** 31)):
            row = (*COMPONENT_ROW[:7], headcount, *COMPONENT_ROW[8:])
