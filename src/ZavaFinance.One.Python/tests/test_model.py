...
from zavafinance.model import (
    ModelRouter, RouteDecision, ToolSelectionError, WITHHELD_RESULT, decision_messages,
    history_messages, read_json, trim_history, validate_decision,
)


def router(response=None):
    result = object.__new__(ModelRouter)
    result.settings = Settings()
    result._client = SimpleNamespace(get_response=AsyncMock(return_value=response))
    return result


def response(*calls, text=""):
    return SimpleNamespace(messages=[Message("assistant", list(calls))], text=text)
...
class NativeValidationTests(unittest.TestCase):
    ...
    def test_declarations_have_no_execution_functions(self):
        declarations = ModelRouter.tool_declarations()
        self.assertEqual({d.name for d in declarations}, {"get_statement", "get_kpi_info", "explore_finance"})
        for declaration in declarations:
            self.assertIsNone(declaration.func)
...
class RouterTests(unittest.IsolatedAsyncioTestCase):
    async def test_one_call_prose_discarded_and_followups_preserved(self):
        model = router(response(Content.from_function_call("c", "get_statement", arguments='{"org":"Nordics"}'),
                                text="Do not return this speculative answer."))
        history = [{"role": "user", "content": "Margin in Q3 2026"}]
        decision = await model.route(history, "and for the Nordics?")
        self.assertEqual(decision, RouteDecision("get_statement", {"org": "Nordics"}, "c"))
        model._client.get_response.assert_awaited_once()
        kwargs = model._client.get_response.call_args.kwargs
        self.assertFalse(kwargs["options"]["parallel_tool_calls"])
        self.assertFalse(kwargs["options"]["store"])
        self.assertEqual(kwargs["options"]["reasoning"], {"effort": "low"})
        self.assertNotIn("temperature", kwargs["options"])
        self.assertEqual(history, [{"role": "user", "content": "Margin in Q3 2026"}])
    ...
    async def test_invalid_response_never_selects_one_arbitrarily(self):
        ...
        for content in calls:
            with self.subTest(content=content), self.assertRaises(ToolSelectionError):
                await router(response(*content)).route([], "question")

    async def test_plain_text_response(self):
        self.assertEqual((await router(response(text="I can help with finance.")).route([], "Hello")).text,
                         "I can help with finance.")
        with self.assertRaises(ToolSelectionError):
            await router(response()).route([], "Hello")
    ...
    async def test_rerank_preserves_ambiguity_rejects_unbound_ids_and_never_stores(self):
        ...
        for text, expected in [('{"ids":["one","two"]}', ["one", "two"]), ('{"ids":["unbound"]}', None),
            ...
            options = model._client.get_response.call_args.kwargs["options"]
            self.assertFalse(options["store"])
            self.assertNotIn("tools", options)
        model = router()
        self.assertEqual(await model.rerank("anything", "org", []), '{"ids":[]}')
        model._client.get_response.assert_not_called()

    async def test_rerank_cancellation_propagates(self):
        model = router()
        model._client.get_response.side_effect = asyncio.CancelledError()
...
class ResponsesHTTPTests(unittest.IsolatedAsyncioTestCase):
    async def test_actual_project_constructor_is_lazy_and_does_not_own_credential(self):
        ...
        model = ModelRouter(Settings(project_endpoint="https://fixture.services.ai.azure.com/api/projects/demo"),
                            credential)
        try:
    ...
    async def test_real_maf_reranker_uses_stateless_structured_response_without_tools(self):
        ...
        async with httpx.AsyncClient(transport=httpx.MockTransport(handle)) as http:
            ...
            model = router()
            model._client = RawOpenAIChatClient(model="gpt-5.4-mini", async_client=sdk)
    ...
    async def test_real_maf_raw_client_wire_and_no_second_request(self):
        ...
        async with httpx.AsyncClient(transport=httpx.MockTransport(handle)) as http:
            ...
            decision = await model.route(history, "show it")
